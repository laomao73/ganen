...
import com.ganen.bean.QueryPaymentRequestBean;
import com.ganen.bean.QueryPaymentResponseBean;
...
public class PaymentApiTest {
	 ...
	 * @throws Exception
	 */
	public static void paymentApi(String no_order,String dt_order,String money_order,String card_no,String acct_name) throws Exception {
		// 连连内部测试环境数据(商户测试期间需要用正式的数据测试，测试时默认单笔单日单月额度50，等测试OK，申请走上线流程打开额度）
		PaymentRequestBean paymentRequestBean = new PaymentRequestBean();
		paymentRequestBean.setNo_order(no_order);
		paymentRequestBean.setDt_order(dt_order);
		paymentRequestBean.setMoney_order(money_order);
		paymentRequestBean.setCard_no(card_no);
		paymentRequestBean.setAcct_name(acct_name);
